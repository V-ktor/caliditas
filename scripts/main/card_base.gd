...
func zoom():
	...
	get_node("TweenScale").interpolate_property(self,"scale",get_scale(),Vector2(0.3,0.3),0.25,Tween.TRANS_LINEAR,Tween.EASE_IN_OUT)
	if (pos!=null):
...
func unzoom():
	if (type=="dead"):
		if (get_node("Animation").get_current_animation()!="fade_out"):
			get_node("Animation").play("fade_out")
	get_node("TweenScale").interpolate_property(self,"scale",get_scale(),Vector2(0.15,0.15),0.25,Tween.TRANS_LINEAR,Tween.EASE_IN_OUT)
	if (pos!=null):
